...
module.exports = app => {
    ...
    const viewDoctorListaMedicamentos = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-lista-medicamentos',{
                page: 'Lista de Medicamentos',
                user,
                message: null
            })
        }).catch(_ => res.status(500).render('500'))
    ...
    const viewDoctorListaPacientes = async (req, res) => {
        await User.findOne({ _id: req.session.user._id }).then(user => {
            res.status(200).render('medico-lista-pacientes',{
    ...
    return { 
        ...
        viewDoctorFinancial,
        viewDoctorQuestions,
        viewDoctorListaMedicamentos,
        viewDoctorListaPacientes,
        viewDoctorTelemedicina,
        viewDoctorResultado,
